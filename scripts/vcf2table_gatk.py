...
def get_default_vcf_parse_dict(rec, sample_name, alt_n):
    ...
    return OrderedDict([
                ("Sample",           sample_name),
                ("Position",         rec.POS),
                ("Type",         rec.ALT[alt_n].type),
                ("Reference",        rec.REF),
                ("Alternative",      rec.ALT[alt_n].value),
                ("Quality",          rec.QUAL),
                ("AlleleFrequency",  rec.INFO["AF"][0]),
                ("TotalReadDepth",   rec.INFO["DP"]),
                ("QalityByDepth",    rec.INFO["QD"]),
                ("No_Ref",           rec.calls[0].data.get("AD")[0]),
                ("No_Alt",           rec.calls[0].data.get("AD")[1]),
