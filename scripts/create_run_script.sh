...
do
	...
	echo "nextflow run ${PWD}/pf.resistance.nf	--sampleid ${sampleid} --outDir ${PWD}/output/${today} --reads \"${PWD}/input/${sampleid}_{1,2}.fastq.gz\" -resume --threads 50 -with-report -with-timeline" >> runscript.sh
done
