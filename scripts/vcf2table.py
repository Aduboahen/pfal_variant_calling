...
# GLOBALS
# error code dict:
ERROR_CODES = {"MultipleCall_Err": 1, "snpEff_Err": 2}
...
def main():
    ...
    n = 0
    for n, vcfdata in enumerate(
        vcf2table(
            args.input,
            args.sample,
            args.annotations,
            args.exclude_so,
            args.remove_duplicate_snpeff,
        )
    ):
        if n == 0:
...
def vcf2table(
    vcfstream, sample_name, parse_annotations, exclude_so, remove_duplicate_snpeff
):
    ...
    with vcfpy.Reader.from_stream(vcfstream) as vcffile:
        for rec in vcffile:
            for alt_n, alt in enumerate(rec.ALT):  # one row per ALT
                alt = (
                    alt.value
                )  # the alt nucleotide is needed for getting the correct annotations

                if sample_name == "CHROM":
                    sample_name = rec.CHROM

                if parse_annotations:
                    for annotations in parse_snpeff_annotations(
                        rec, alt, exclude_so, remove_duplicate_snpeff, HGVS_parse_regx
                    ):
...
def get_default_vcf_parse_dict(rec, sample_name, alt_n):
    ...
    return OrderedDict(
        [
            ("sample", sample_name),
            ("position", rec.POS),
            ("var_type", rec.ALT[alt_n].type),
            ("reference", rec.REF),
            ("alternative", rec.ALT[alt_n].value),
            ("quality", rec.QUAL),
            # ("Allele Frequency", rec.INFO["AF"]),
            ("total_read_depth", rec.INFO["DP"]),
            ("ref_forw", rec.INFO["DP4"][0]),
            ("ref_rev", rec.INFO["DP4"][1]),
            ("alt_forw", rec.INFO["DP4"][2]),
            ("alt_rev", rec.INFO["DP4"][3]),
...
def parse_snpeff_annotations(
    ...
    try:
        ...
        for annotation in rec.INFO["ANN"]:
            ...
            if (
                annotation[0] == alt
            ):  # check if this is the correct annotation for this alt
                if (
                    annotation[1] not in EXCLUDE_SO_TERM_LIST and exclude_so
                ) or not exclude_so:
                    if annotation[10]:
                        ...
                        # Shorthand = "-"
                    if (
                        remove_duplicate_snpeff and not HGVS in annotation_mem
                    ) or not remove_duplicate_snpeff:
...
def get_hgvs_parse_patterns():
    ...
    return {
        "aa_sub": re.compile(
            rf"^\w+:p\.({aa_list})(\d+)({aa_list})$"
        )  # searches for <Gene_Name>:p.<three letter AA list><coordinates><three letter AA list>
    }
...
def hgvs_aasub2shorthand(hvgs, regx_pattern):
    ...
    if parsed_hvgs:
        ...
        return "".join(
            [AA_CODES[parsed_hvgs[1]], parsed_hvgs[2], AA_CODES[parsed_hvgs[3]]]
        )
    else:
        return "-"
...
def get_command_line_arguments():
    ...
    argp = argparse.ArgumentParser(
        description="Reformats vcffiles, only takes plain text vcf files from stdin, outputs table on stdout WARNING! will error out when more than one ALT is present!"
    )
    argp.add_argument(
        "input",
        default=stdin,
    ...
    argp.add_argument(
        "-a", "--annotations", action="store_true", help="""parse snpEff annotations"""
    )
    argp.add_argument(
        "-d",
        "--remove_duplicate_snpeff",
        action="store_true",
        help="""snpeff has a tendency to report duplicates,
                this is most likely caused by the covid genome having two
                higly similar/ the same features that were annotated.
                say GU280_gp01 and GU280_gp01.2
                """,
    )
    argp.add_argument(
        "-e",
