...
bed_file         = "$projectDir/reference/amplicon.bed"
...
process mapping {
    ...
    output:
      file ("${sampleid}.bam")  into bam1A, bamDepth
...
process "variant calling"{
    ...
    output:
      file ("${sampleid}.vcf") into vcFiltersnp, vcFilterindel, vcfNonCov, vcfCon
    script:
    """
      bcftools mpileup -a DP -B -O u -m 4 --threads ${threads} -f \
      -R ${bed_file}
      ${reference} ${bam}\
...
process "filter snps"{
    ...
    publishDir outDir + "/variants", mode: 'copy'
    input:
      file vcf from vcFiltersnp
    output:
      file ("${sampleid}.snps.vcf") into vcfAnnot
    script:
    """
...
process "filter indels"{
    ...
    publishDir outDir + "/variants", mode: 'copy'
    input:
      file vcf from vcFilterindel
    output:
      file ("${sampleid}.indel.vcf")
    script:
    """
...
process 'annotation'{
    ...
    script:
    """
      snpEff ann -v -noShiftHgvs -ud 0 -strict -s \
